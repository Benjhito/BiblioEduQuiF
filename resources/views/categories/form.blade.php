<div class="d-flex justify-content-center">
    <form class="w-50 bg-light border border-dark rounded p-4 mb-3 mx-5" method="POST" action="{{ $route }}" enctype="multipart/form-data">
        ...
        <h5 class="text-center"><b>{{ $update ? 'Editar Categoría' : 'Nueva Categoría' }}</b></h5>

        <div class="form-row">
            <div class="form-group col-2">
                <label class="col-form-label text-secondary" for="code" id="code__label"><b>Cód.Int.</b></label>
                <input class="string form-control" id="code" name="code" type="text" value="{{ $category->code }}" readonly="on" />
            </div>

            <div class="form-group col-10">
                <label class="col-form-label text-secondary" for="name" id="name__label"><b>Nombre</b></label>
                <input class="string form-control" id="name" name="name" type="text" value="{{ old('name') ?? $category->name }}" maxlength="30" autofocus="on" />
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group col-12">
                ...
                <input class="string form-control" id="descrip" name="descrip" type="text" value="{{ old('descrip') ?? $category->descrip }}" maxlength="40" />
            </div>
        </div>
        ...
        @if($category->image)
            <div class="form-row">
                <div class="form-group col-12 text-center">
                    ...
                    <img width="120px" src="/storage/{{ $category->image }}">
        ...
        <div class='d-flex justify-content-center pt-2'>
            <a class="button btn btn-secondary mr-5" href="{{ route('categories.index') }}" title="Regresa a la página de Categorías">Cancelar</a>
