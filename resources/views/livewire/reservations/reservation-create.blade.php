<div class="web2py_grid">
    <center><h5><b class="text-warning">Reservas | Nueva Reserva</b></h5></center>
    <div class="web2py_console">
        ...
        @else
            <div class="d-flex">
                <div class="text-warning"><b>{{ 'Socio: ' . $member->code . ' - ' . $member->fullName() }}</b></div>
            </div>
        @endif
            ...
            <form wire:submit.prevent="addBook" method="POST">
                ...
                <div class="form-row w-100">
                    ...
                    <div class="form-group col-12 col-md-1 ml-3">
                        ...
                        <input class="btn btn-warning text-white @if (!$book) disabled @endif" type="submit" value="Agregar" />
                    </div>

                    @if($reservation->status == 'Pendiente' && $reservation->items()->count() > 0)
                        <div class="form-group col-12 col-md-1 ml-3">
                            <label class="invisible">Confirmar</label>
                            <a wire:click="saveReservation" wire:loading.attr="disabled" href="#" class="button btn text-white px-2 py-1" style="background-color:lightseagreen" title="Confirma la Reserva">Confirmar</a>
                        </div>
                    @endif

                    <div class="form-group col-12 col-md-1 ml-4">
                        <label class="invisible">Volver</label>
                        <a class="button btn btn-secondary px-3 py-1" href="{{ route('reservations.create') }}" title="Ir a la página de Nueva Reserva">Volver</a>
                    </div>
                </div>
            </form>

            @if($reservation)
                @livewire('reservations.reservation-items', ['reservation' => $reservation])
            @endif
        @endif
    </div>
</div>
